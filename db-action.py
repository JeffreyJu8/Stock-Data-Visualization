...
def create(fn):
    ...
    conn = sl.connect(db)
    curs = conn.cursor()
    stmts = ["CREATE TABLE IF NOT EXISTS Tesla (" + header + ")",
             "CREATE TABLE IF NOT EXISTS Amazon (" + header + ")"
             ]
...
def main():
    ...
    create("csv_files/tesla.csv")
    create("csv_files/AMZN.csv")
    store_data("csv_files/tesla.csv", "tesla_stock")
    store_data("csv_files/AMZN.csv", "amazon_stock")
